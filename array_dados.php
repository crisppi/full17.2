<?php

$dados_acomodacao = ["UTI", "Apto", "Enfermaria", "Semi"];
$dados_UTI = ["Insuficência respiratória", "Choque cardiogênico", "Choque séptico", "Distúrbio metabólico"];
$dados_saps = ["0-20", "21-40", "41-61", "61-80", ">81"];
$dados_especialidade = ["Clínica", "Pediatria", "Ginecologia", "Cirurgia Geral", "Cirurgia Vascular", "Cirurgia Torácica"];
$criterios_UTI = ["Clínica", "Pediatria", "Ginecologia", "Cirurgia Geral", "Cirurgia Vascular", "Cirurgia Torácica"];
$dados_grupo_pat = ["Cardiológica", "Pediátrica", "Oncológica", "Urológica", "Infecção"];
$dados_tipo_evento = ["Úlcera pressão", "Queda", "Broncoaspiração", "Infecção hospitalar", "Infecção cirúrgica"];
$dados_alta = ["Melhorado", "Transferido", "Óbito"];
$cargo_user = ["Med_auditor", "Enf_Auditor", "Adm", "Diretoria", "Gerencia", "Secretaria"];
